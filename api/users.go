...
func FetchCoupon(ctx *gin.Context) {
	...
	if err == nil {
		...
		message := RobbitMqService.NewMessage(claims.Username, coupon)
		if err := RobbitMqService.Rabbitmq.PublishSimple(message.JsonToStr()); err != nil {
			log.Printf("false 加入消息队列失败：" + err.Error())
		}
